...
impl Octant {
    ...
    #[inline]
    fn point_to_octant0(&self, p: Point) -> Point {
        match self.0 {
            0 => Point::new(p.x, p.y),
            1 => Point::new(p.y, p.x),
            2 => Point::new(p.y, -p.x),
    ...
    #[inline]
    fn point_from_octant0(&self, p: Point) -> Point {
        match self.0 {
            0 => Point::new(p.x, p.y),
            1 => Point::new(p.y, p.x),
            2 => Point::new(-p.y, p.x),
...
impl Bresenham {
    ...
    pub fn new(start: Point, end: Point) -> Bresenham {
        ...
        let start = octant.point_to_octant0(start);
        let end = octant.point_to_octant0(end);
    ...
    pub fn advance(&mut self) -> Point {
        ...
        self.octant.point_from_octant0(p)
    }
}
